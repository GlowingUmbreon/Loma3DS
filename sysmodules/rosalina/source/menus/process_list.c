...
#define TRY(expr) if(R_FAILED(res = (expr))) goto end;
    ...
    if(R_SUCCEEDED(res))
    {
        res = FSUSER_CreateDirectory(archive, fsMakePath(PATH_ASCII, "/loma/dumps"), 0);
        if((u32)res == 0xC82044BE) // directory already exists
            res = 0;
        res = FSUSER_CreateDirectory(archive, fsMakePath(PATH_ASCII, "/loma/dumps/memory"), 0);
        if((u32)res == 0xC82044BE) // directory already exists
            res = 0;
    ...
    sprintf(filename, "/loma/dumps/memory/%.8s_0x%.8lx_%.4u-%.2u-%.2uT%.2u-%.2u-%.2u.bin", name, (u32)start, year, month, days, hours, minutes, seconds);
